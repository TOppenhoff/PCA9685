...
void loop()
{
	...
	driver.getLed(PCA9685_LED0).fullOnAndWrite();
	driver.getLed(PCA9685_LED1).fullOnAndWrite();
	driver.getLed(PCA9685_LED2).fullOnAndWrite();
	driver.getLed(PCA9685_LED3).fullOnAndWrite();
	...
	driver.getLed(PCA9685_LED0).fullOffAndWrite();
	driver.getLed(PCA9685_LED1).fullOffAndWrite();
	driver.getLed(PCA9685_LED2).fullOffAndWrite();
	driver.getLed(PCA9685_LED3).fullOffAndWrite();
	...
	for (i = 0; i < PCA9685_MAX_VALUE; i=i+8)
	{
		...
		driver.getLed(PCA9685_LED0).setValueAndWrite(i);
		delay(5);
	}
	...
	for (i = 0; i < 1000; ++i)
	{
		driver.getLed(PCA9685_LED0).setValueAndWrite(random(0,5)*1024);
		driver.getLed(PCA9685_LED1).setValueAndWrite(random(0, 5) * 1024);
		driver.getLed(PCA9685_LED2).setValueAndWrite(random(0, 5) * 1024);
		driver.getLed(PCA9685_LED3).setValueAndWrite(random(0, 5) * 1024);
		delay(5);
	}
}
